...
var JsonFormatter = {
  stringify: (cipherParams: any) => {
    ...
    var jsonObj = { ct: cipherParams.ciphertext.toString(CryptoJS.enc.Base64), iv: cipherParams.iv.toString() };
...
export const encrypt = (message: string, password: string = 'EasyStream'): string => {
  // Encrypt the message
...
export const QRCode = ({ value, className }: any) => (
  <ResponsiveSvgWrapper>
    <QRCodeReact className={className} renderAs="svg" value={value}/>
  </ResponsiveSvgWrapper>
)
